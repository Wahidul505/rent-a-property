...
const Navbar: FC = () => {
    ...
    const authItems: JSX.Element = <div className='flex flex-col md:flex-row gap-2 md:gap-4 md:mt-3'>
        {
            user?.email ?
                <div className="dropdown dropdown-end dropdown-hover">
                    <label tabIndex={0} className="btn m-1">{user?.name && user?.name[0]}</label>
                    <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52 mt-3 md:mt-0">
                        <li><Link to='/dashboard' className='font-semibold'>Dashboard</Link></li>
                        <li className='pl-3 badge badge-primary badge-outline mb-2 mt-1'>{email}</li>
                        <li><button onClick={handleSignOut} className='btn btn-md btn-primary btn-outline'>SignOut</button></li>
                    </ul>
                </div>
                :
                <>
        ...
        <div className="navbar bg-white fixed top-0 z-40">
            <div className="">
                <div className="dropdown">
                    <label tabIndex={0} className="btn btn-sm md:btn-md btn-ghost md:hidden">
                ...
                <Link to='/' className="btn btn-sm md:btn-md btn-ghost normal-case text-lg md:text-2xl">Rent a Property</Link>
            </div>
