...
const RentApplications: FC<Props> = ({ id }) => {
    ...
    const { data: applications, isLoading, refetch } = useQuery(['rent-applications', id], () => fetch(`https://rent-a-property.onrender.com/applications/${id}`, {
        headers: {
    ...
    useEffect(() => {
        if (currentId && currentStatus && user) {
            fetch(`https://rent-a-property.onrender.com/applications?id=${currentId}&status=${currentStatus}&email=${user?.email}`, {
                method: 'PATCH',
                headers: {
                    'authorization': `Bearer ${user.token}`
                }
